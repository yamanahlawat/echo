...
def get_cosine_annealing_warm_restarts_scheduler(
    optimizer: Optimizer, T_0: int, T_mult: int = 1, eta_min: float = 1e-6, verbose: bool = True
):
...
def get_cosine_annealing_scheduler(optimizer: Optimizer, T_max: int, eta_min: float = 1e-6, verbose: bool = True):
